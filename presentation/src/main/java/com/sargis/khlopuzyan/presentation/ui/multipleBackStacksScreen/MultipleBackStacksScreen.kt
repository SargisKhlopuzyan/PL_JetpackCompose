...
//            startDestination = MBSBtmNavScreen.HomeScreen.route
            startDestination = MBSBtmNavScreen.ChatScreen.route
        ) {
...
fun HomeNavHost() {
    ...
    NavHost(homeNavController, startDestination = "${homeRoute}1") {
        for (i in 1..10) {
            val homeChildScreenRoute = "$homeRoute$i"
            val homeNextChildScreenRoute = "$homeRoute${(i + 1)}"
            composable(homeChildScreenRoute) {
                GenericScreen(
                    text = "Home $i",
                    onNextClick = {
                        if (i < 10) {
                            homeNavController.navigate(homeNextChildScreenRoute)
                        }
                    }
                )
            }
...
fun ChatNavHost() {
    ...
    NavHost(chatNavController, startDestination = "${chatRoute}1") {
        for (i in 1..10) {
            val chatChildScreenRoute = "$chatRoute$i"
            val chatNextChildScreenRoute = "$chatRoute${(i + 1)}"
            composable(chatChildScreenRoute) {
                GenericScreen(
                    text = "Chat $i",
                    onNextClick = {
                        if (i < 10) {
                            chatNavController.navigate(chatNextChildScreenRoute)
                        }
                    }
                )
            }
...
fun SettingsNavHost() {
    ...
    NavHost(settingsNavController, startDestination = "${settingsRoute}1") {
        for (i in 1..10) {
            val settingsChildScreenRoute = "$settingsRoute$i"
            val settingsNextChildScreenRoute = "$settingsRoute${(i + 1)}"
            composable(settingsChildScreenRoute) {
                GenericScreen(
                    text = "Settings $i",
                    onNextClick = {
                        if (i < 10) {
                            settingsNavController.navigate(settingsNextChildScreenRoute)
                        }
                    }
                )
            }
